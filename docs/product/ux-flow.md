...
2. **Choose next action from Work List**
   - Open piece
   - Create new piece
   - Open Backup / Restore when the user needs a full local data backup before version updates
...
7. **Backup / Restore**
   - Export a complete JSON backup of all local works, annotations, and layer visibility state
   - Restore from a JSON backup after explicit replace-all warning and schema validation
...
- Song UX is out of MVP; if song records are surfaced, they are disabled/not implemented and do not route to song UX
- Markdown import/export UX is out of MVP
- JSON backup/restore UX is in MVP as a data-safety flow
