...
Note: `chord` and `meter` layers are visual channels driven by `SongCell` data in song content. Song UX is out of MVP, so MVP text/poem UI exposes only `breath`, `intention`, and `comments` controls while preserving `chord` and `meter` in the domain/data model for MVP2.

### Rules
- Layers are fixed in the domain/data model. Users cannot create, rename, or delete layers in MVP.
...
|---|---|---|---|
| `breath` | `BreathContent` | `mark` must be `'S'` or `'L'`, always user-chosen | No |
| `intent` | `NoteAnnotationContent` | `shortNote` non-empty, personal notes | Never |
| `comment` | `NoteAnnotationContent` | `shortNote` non-empty | Future yes |

`shortNote` is displayed as a floating handwritten note above the annotated text in the reading surface.
`extendedNote` is optional and shown in the sidebar detail panel when the annotation is selected.
