...
- This is achieved via pre-rendered PieceSnapshot loaded from IndexedDB — no React render pipeline over the reading content on open
...
- The Tiptap editor must feel native — keystrokes must register with no perceptible lag
...
### DOM size
- The visualization view injects static HTML — no live React components over the text content
...
### Memory
- No memory leaks from visualization view lifecycle or injected snapshot teardown
...
### Internal metadata stripping on Markdown export
- Exported `.md` files must never contain annotation metadata, target metadata, adapter-specific state, or legacy anchor tags (`<!--a1s-->` etc.)
- This must be verified: export output must be clean Markdown derived from structured content
...
- In MVP this applies to visualization mode, where annotation layers are exposed. Editing mode does not expose annotation layers or annotation actions; if a future editing annotation view is added, the same override remains required.
...
| Annotation targets per piece | ~200 | Same practical constraint as annotations |
