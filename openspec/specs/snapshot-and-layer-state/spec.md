...
Domain layer set SHALL remain fixed to 5 layers:
- `chord`
- `meter`
- `breath`
- `intention`
...
MVP text/poem UX SHALL expose only annotation-driven layer controls (`breath`, `intention`, `comments`). `chord` and `meter` remain in the snapshot/layer model so song content can be added in MVP2 without a core model refactor.
...
CSS hide rules SHALL support both:
...
**WHEN** user toggles an exposed annotation-driven layer
