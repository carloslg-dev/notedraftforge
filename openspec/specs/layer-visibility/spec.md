...
## Purpose
Define unified per-piece visibility behavior for fixed domain layers across annotation-driven MVP rendering and song-cell-driven rendering prepared for MVP2.

---

## Requirements

### LV-REQ-01 — Fixed domain layer set
The domain/data model SHALL provide exactly 5 fixed layers:
- `chord`
- `meter`
- `breath`
- `intention`
...
MVP text/poem UX SHALL expose only annotation-driven layer toggles:
- `breath`
- `intention`
- `comments`

`chord` and `meter` remain stored layer keys for song compatibility but SHALL be hidden/disabled in MVP UI unless song UX is explicitly implemented.
...
**GIVEN** a future song visualization mode with song content
