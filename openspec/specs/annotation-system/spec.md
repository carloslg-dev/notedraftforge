...
### AS-REQ-05 — Annotation content model
Annotation content SHALL conform to the `AnnotationContent` union from the domain model:

- `BreathContent` → `{ mark: 'S' | 'L' }` — marks a breath pause at the target. `mark` must be `'S'` (short) or `'L'` (long); always user-chosen.
- `NoteAnnotationContent` for `intent` and `comment` → `{ shortNote: string; extendedNote?: string }`:
  - `shortNote` — mandatory, non-empty; a brief phrase shown floating above the annotated text in the reading surface (Caveat handwriting font).
  - `extendedNote` — optional, non-empty when present; a longer note shown in the sidebar detail panel when the annotation is selected.

The system SHALL reject any annotation creation or update where:
- `BreathContent.mark` is not `'S'` or `'L'`
- `shortNote` is empty or missing for `intent` / `comment` kinds
...
4. The annotation modal collects:
   ...
   - For `intent` / `comment`: a `shortNote` text input (mandatory) and an expandable `extendedNote` text area (optional)
5. On confirm, the annotation is validated and persisted.
...
### AS-REQ-10 — needsReview trigger and behavior
...
### AS-SCN-07 — Create annotation via selection toolbar
**GIVEN** a piece in visualization mode
**WHEN** user selects text and the selection toolbar appears, then taps "Intent"
**THEN** the annotation modal opens pre-configured for `intent` kind with empty `shortNote` and hidden `extendedNote` field.

### AS-SCN-08 — Annotation modal validates shortNote
**GIVEN** the annotation modal is open for an `intent` annotation
**WHEN** user submits with an empty `shortNote`
**THEN** the modal shows a validation error and does not persist the annotation.

### AS-SCN-09 — Breath annotation uses mark selector
**GIVEN** the annotation modal is open for a `breath` annotation
**WHEN** user selects `S` (short) and confirms
**THEN** the system persists `BreathContent { mark: 'S' }`.

### AS-SCN-10 — shortNote visible in reading surface
**GIVEN** a piece with an `intent` annotation having `shortNote: "más lento"`
**WHEN** visualization mode renders with intention layer active
**THEN** "más lento" floats above the annotated text in the reading surface.

### AS-SCN-11 — extendedNote shown in sidebar
**GIVEN** a piece with an `intent` annotation having `extendedNote: "Reducir el tempo hasta casi detenerse"`
**WHEN** user selects the annotation in visualization mode
**THEN** the sidebar (or detail panel) shows the extended note text.

### AS-SCN-12 — Selection Refinement adjusts boundary before annotation
**GIVEN** a user selects text on mobile in visualization mode and taps "Refine" in the selection toolbar
**WHEN** the Selection Refinement modal opens
**THEN** the user can nudge the selection boundary char-by-char; on confirm, the adjusted selection is used for the annotation target without committing any annotation yet.
